...
int main(int argc, char *argv[]){
	...
	for(Int_t Nu_bin = 0; Nu_bin < N_Nu; Nu_bin++){
		...
		TFile *acc = new TFile(Form("acc/SM1Dfout_"+Nuclei_Type+"_%dnubin%d_Ebins%d_step%f.root", N_Nu, Nu_bin, nbins, step_E*1000)); // Acceptance files
		...
		for (int shift = 0; shift <= nshift_E; ++shift){ 
			...
			// Saving fit
			fileFit->cd();
			h->SetName(Form("fit_"+Nuclei_Type+"_shift%d_nubin%d", shift, Nu_bin));
			h->Write();
