...
// DATA NTUPLE
  ...
  TFile *plots = new TFile(Form("output/3Dfout_"+Nuclei_Type+"_%dnubin%d.root", N_Nu, Nu_bin),"RECREATE");
  //cout << "OK" << endl;
  ...
  for (int j = 0; j < N_Q2; j++){
    ...
    for (int k = 0; k < N_Phi; k++){
      ...
      hDA->SetName(Form("acceptance_histoD%d%d%d", Nu_bin, j, k)); //AQUI CHECK NAME
      hDA->Write();
      ...
      for(int shift = shift_min; shift <= shift_max; shift++){
        ...
        hA->SetName(Form("acceptance_histo"+Nuclei_Type+"_shift%d_%d%d%d", shift, Nu_bin, j, k)); //AQUI
        hA->Write();
