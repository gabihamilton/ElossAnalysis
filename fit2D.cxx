...
int main(int argc, char *argv[]){
	...
	TFile *fileFit = new TFile(Form("fits/FIT2D_"+Nuclei_Type+"_%dnubins_cheb%d_Ebins%d_step%f.root", N_Nu, n, nbins, step_E*1000), "RECREATE");
	...
	for(Int_t Nu_bin = 0; Nu_bin < N_Nu; Nu_bin++){
		...
		for (int Q2_bin = 0; Q2_bin < N_Q2; Q2_bin++){
			...
			TH1F *DAcc = (TH1F*)acc->Get(Form("acc_D_nubin%d_q2bin%d", Nu_bin, Q2_bin)); // Getting the right acc histogram
			...
			for (int shift = 0; shift <= nshift_E; ++shift){ 
				...
				TH1F *h = (TH1F*)acc->Get(Form("acc_"+Nuclei_Type+"_shift%d_nubin%d_q2bin%d", shift, Nu_bin, Q2_bin)); // Getting the right acc histogram
				...
				// Saving fit
				fileFit->cd();
				h->SetName(Form("fit_"+Nuclei_Type+"_shift%d_nubin%d_q2bin%d", shift, Nu_bin, Q2_bin));
				h->Write();
