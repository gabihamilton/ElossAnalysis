...
int main(int argc, char *argv[]){
	...
	for(Int_t Nu_bin = 0; Nu_bin < N_Nu; Nu_bin++){
		...
		TFile *acc = new TFile(Form("output/fout_"+Nuclei_Type+"_nubin%d.root", Nu_bin)); // Acceptance files
		...
		// Starting Loop over Q2 bins for Deuterium
		for (int Q2_bin = 0; Q2_bin < N_Q2; ++Q2_bin){
			...
			TH1F *DAcc = (TH1F*)acc->Get(Form("acceptance_histoD%d%d", Nu_bin, Q2_bin)); // Getting the right acc histogram
			...
			funcD->GetParameters(&parD[Q2_bin][0]);  // Saving Fit Parameters
		...
		for (int shift = 0; shift <= nshift_E; ++shift){ 
			...
			for (int Q2_bin = 0; Q2_bin < N_Q2; ++Q2_bin){
    			...
    			TH1F *h = (TH1F*)acc->Get(Form("acceptance_histo"+Nuclei_Type+"_shift%d_%d%d", shift, Nu_bin, Q2_bin)); // Getting the right acc histogram
