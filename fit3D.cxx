...
int main(int argc, char *argv[]){
	...
	TFile *fileFit = new TFile(Form("fits/FIT3D_"+Nuclei_Type+"_%dnubins_cheb%d_Ebins%d.root", N_Nu, n, nbins), "RECREATE");
	...
	for(Int_t Nu_bin = 0; Nu_bin < N_Nu; Nu_bin++){
		...
		for (int Q2_bin = 0; Q2_bin < N_Q2; Q2_bin++){
			...
			for (int Phi_bin = 0; Phi_bin < N_Phi; Phi_bin++){
				...
				TH1F *DAcc = (TH1F*)acc->Get(Form("acc_D_nubin%d_q2bin%d_phi%d", Nu_bin, Q2_bin, Phi_bin)); // Getting the right acc histogram
				...
				// Saving fit
				fileFit->cd();
				DAcc->SetName(Form("fitD_"+Nuclei_Type+"_nubin%d_q2bin%d_phibin%d", Nu_bin, Q2_bin, Phi_bin));
				DAcc->Write();
				...
				for (int shift = 0; shift <= nshift_E; ++shift){ 
					...
					TH1F *h = (TH1F*)acc->Get(Form("acc_"+Nuclei_Type+"_shift%d_nubin%d_q2bin%d_phibin%d", shift, Nu_bin, Q2_bin, Phi_bin)); // Getting the right acc histogram
					...
					// Saving fit
					fileFit->cd();
					h->SetName(Form("fit_"+Nuclei_Type+"_shift%d_nubin%d_q2bin%d_phibin%d", shift, Nu_bin, Q2_bin, Phi_bin));
					h->Write();
